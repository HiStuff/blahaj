...
import { REST, RESTPostAPIChatInputApplicationCommandsJSONBody, RESTPostAPIContextMenuApplicationCommandsJSONBody, Routes, Snowflake } from "discord.js";
...
export async function deployGlobal() {
    ...
    client.commands.forEach(command => {
        if (!command.flags.includes(CommandFlags.DevOnly)) commands.push(command.data.toJSON());
    });
	...
	log.success(`Successfully reloaded ${commands.length} application commands.`);
}
