...
export class LanguageManager {
            ...
            fs.readdir(path.join(__dirname, this.folderPath), (err, files) => {
                ...
                files.forEach(file => {
                    if (file.endsWith(".json")) {
                        fs.readFile(path.join(__dirname, this.folderPath, file), (err, data) => {
                            if (err) {
                                throw err;
                            }
                            const lang = JSON.parse(data.toString()) as Language;
    ...
    getNameLocalizations(key: string) {
        ...
        Object.keys(this.languages).forEach(language => {
            ...
            if (!lang.localizations[key]) {
                log.error(`${lang.locale == "en-US" ? "[SEVERE] " : ""}Couldn't get name localization. Language: ${lang.locale}, key: ${key}`)
                return;
            }
            localizations[lang.locale as keyof Record<Locale, string | null>] = lang.localizations[key].name
        });
    ...
    getDescriptionLocalizations(key: string) {
        ...
        Object.keys(this.languages).forEach(language => {
            const lang = this.languages[language] || this.languages["en-US"];
            if (!lang.localizations[key]) {
                log.error(`${lang.locale == "en-US" ? "[SEVERE] " : ""}Couldn't get description localization. Language: ${lang.locale}, key: ${key}`)
                return;
            }
            localizations[lang.locale as keyof Record<Locale, string | null>] = lang.localizations[key].description
        });
        return localizations;
    }
}