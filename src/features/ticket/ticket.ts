...
let openButton: ButtonBuilder = new ButtonBuilder()
	.setLabel("Otwórz")
	.setCustomId("openticket")
	.setStyle(ButtonStyle.Primary)
	.setEmoji("🚪");
let closeButton: ButtonBuilder = new ButtonBuilder()
	.setLabel("Zamknij")
...
let deleteButton: ButtonBuilder = new ButtonBuilder()
	.setLabel("Usuń")
	.setCustomId("deleteticket")
	.setStyle(ButtonStyle.Danger)
	.setEmoji("🗑️");

export async function getTicketFromChannel(
	bot: FembotClient,
	guild: Guild,
	channel: TextChannel,
) {
...
export async function handleCustomMenuButtonClick(
	client: FembotClient,
	...
	if (!(await ticket.init(interaction.guild.id))) {
...
export class Ticket {
	...
	async init(guildId: string) {
		this.theme = await database.ticketTheme.findFirst({
			where: { guildId: guildId, id: this.themeKey },
		});
		return this.theme;
	}
	async createTicket(
		...
		if (!categoryId)
			throw new Error(
				`Ten serwer nie ma wybranej kategorii do której trafiają tickety.\nUżyj komendy \`/ticket pomoc\` aby dowiedzieć się, jak skonfigurować funkcje ticketów.`,
			);
		if (!this.theme)
			throw new Error(
				`Wybrany temat ticketa nie istnieje. (${this.themeKey})`,
			);
		let json = {
		...
		await this.channel.permissionOverwrites.edit(this.author, {
			SendMessages: true,
			ViewChannel: true,
		});
		...
		if (answers) {
			...
			} else themeJson.embeds = [answersEmbed.toJSON()];
		}
	...
	async close(bot: FembotClient, interaction?: ButtonInteraction) {
		...
		if (interaction) {
			...
			await this.channel.send("Ticket zostanie zamknięty za 5 sekund.");
		}
		setTimeout(async () => {
			if (!this.channel) return;
			await this.channel.send({
				content: `Ticket został zamknięty przez ${interaction?.user || "bota"}.`,
				components: [
	...
	async open(bot: FembotClient, interaction?: ButtonInteraction) {
		if (!this.channel) return;
		await this.channel.send({
			content: `Ticket został otwarty przez ${interaction?.user || "bota"}.`,
			components: [
				new ActionRowBuilder<ButtonBuilder>().addComponents(
					closeButton,
				),
	...
	async delete(bot: FembotClient, interaction?: ButtonInteraction) {
		...
		if (interaction) {
			...
			await this.channel.send("Ticket zostanie usunięty za 5 sekund.");
		}
		setTimeout(async () => {
			if (!this.channel) return;
			await this.channel.send(
				`Ticket został usunięty przez ${interaction?.user || "bota"}.`,
			);
