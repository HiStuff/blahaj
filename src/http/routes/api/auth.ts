...
export default async function (fastify: FastifyInstance, opts: Object) {
    fastify.get("/auth", async (req: FastifyRequest<{ Querystring: { code: string } }>, reply: FastifyReply) => {
        const { code } = req.query;
        const redirect_uri = new URL(config.oauth2_link).searchParams.get("redirect_uri")
        if (code) {
            if (!redirect_uri) { return reply.redirect(config.oauth2_link); };
            ...
            if (!("access_token" in data && "token_type" in data)) return reply.redirect(config.oauth2_link);
