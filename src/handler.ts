...
const contextMenuCommandsFolderPath = path.join(
	__dirname,
...
export async function loadCommands() {
	for (const folder of commandFolders) {
		...
		const commandFiles = fs
			...
			.filter((file) => file.endsWith(deployed ? ".js" : ".ts"));
		for (const file of commandFiles) {
			const filePath = path.join(commandsPath, file);
			const command = (await import(pathToFileURL(filePath).href))
				.default;
			if (!command) {
				log.warn(`The command at ${filePath} is corrupted.`);
				continue;
			}
...
export async function loadContextMenuCommands() {
	...
	const commandFiles = fs
		.readdirSync(contextMenuCommandsFolderPath)
		.filter((file) => file.endsWith(deployed ? ".js" : ".ts"));
	for (const file of commandFiles) {
		const filePath = path.join(contextMenuCommandsFolderPath, file);
		const command = (await import(pathToFileURL(filePath).href)).default;
		if (!command) {
			log.warn(`The context menu command at ${filePath} is corrupted.`);
			continue;
		}
		if ("data" in command && "execute" in command) {
			if (command.flags.includes(CommandFlags.Disabled)) continue;
			...
			log.warn(
				`The context menu command at ${filePath} is missing a required "data" or "execute" property.`,
			);
		}
	}
	//}
...
export async function loadButtons() {
	...
	for (const file of buttonsFiles) {
		...
		if (!button) {
			log.warn(`The button at ${filePath} is corrupted.`);
			continue;
		}
		if ("customId" in button && "clicked" in button) {
			...
			log.warn(
				`The button at ${filePath} is missing a required "customid" or "clicked" property.`,
			);
		}
	}
}

export async function handleChatInputCommandInteraction(
	...
	try {
		if (command.flags.includes(CommandFlags.DevOnly) && !user.developer) {
			await interaction.reply({
				embeds: [createErrorEmbed(Error.COMMAND_USER_NOT_A_DEVELOPER)],
		...
		if (command.requiredPermission) {
			if (
				!interaction.memberPermissions?.has(command.requiredPermission)
			) {
				await interaction.reply({
					embeds: [createErrorEmbed(Error.NO_USER_PERMISSIONS)],
					flags: MessageFlags.Ephemeral,
				});
				return;
			}
...
export async function handleContextMenuCommandInteraction(
