...
export default {
	data: new SlashCommandBuilder()
		...
		.setDescriptionLocalizations(
			client.lang.getDescriptionLocalizations("ping"),
		),
	...
	async execute(interaction: ChatInputCommandInteraction) {
		...
		if (!interaction.channel?.isTextBased()) {
			await interaction.reply({
				embeds: [createErrorEmbed(Error.COMMAND_CHANNEL_IS_TEXT)],
			});
			return;
		}
		if (!interaction.channel.isSendable()) {
			await interaction.reply({
				embeds: [createErrorEmbed(Error.CHANNEL_NOT_SENDABLE)],
			});
			return;
		}
		const msg = await interaction.channel.send(
			client.lang.getResponse(interaction.locale, "ping_checking"),
		);
