	...
	async execute(interaction: ChatInputCommandInteraction) {
		...
		const ownerUsername = (
			(client.application?.owner ||
				(await client.users.fetch(config.ownerId))) as User
		).username;
		const infoEmbed = new EmbedBuilder()
			.setTimestamp(new Date())
			.addFields(
				{
					name: client.lang.getResponse(
...
[${client.lang.getResponse(interaction.guildLocale, "info_advertise")}](${config.invite_link})
				...
				},
				{
					...
					value: timeToString(process.uptime() * 1000),
				},
				{
					name: client.lang.getResponse(
						interaction.guildLocale,
					...
					value: client.version.build_date
						? new Date(client.version.build_date).toLocaleString(
