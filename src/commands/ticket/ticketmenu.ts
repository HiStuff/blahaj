...
function getEmbedTitle(interaction: ChatInputCommandInteraction) {
	if (interaction.options.getSubcommand(true) == "custom") {
		return interaction.options.getString("embedtitle", true);
	} else {
		return `✉️ ${client.lang.getOther(interaction.guildLocale, "open_ticket_embed_title")}`;
	}
}

function getEmbedDescription(interaction: ChatInputCommandInteraction) {
	if (interaction.options.getSubcommand(true) == "custom") {
		return interaction.options.getString("embeddescription", true);
	} else {
		return client.lang.getOther(
			interaction.guildLocale,
			"open_ticket_embed_description",
		);
	}
}
...
function getButtonId(interaction: ChatInputCommandInteraction) {
	if (interaction.options.getSubcommand(true) == "custom") {
		return "cmt-" + interaction.options.getString("themeid", true);
	} else {
		return "createticketmenu";
	}
}
...
export default {
	data: new SlashCommandBuilder()
		...
		.setDescriptionLocalizations(
			client.lang.getDescriptionLocalizations("ticketmenu"),
		)
		.addSubcommand((input) =>
			input
				.setName("custom")
				.setNameLocalizations(
					client.lang.getNameLocalizations("ticketmenu_custom"),
				)
				.setDescription("Custom menu.")
				.setDescriptionLocalizations(
					client.lang.getDescriptionLocalizations(
						"ticketmenu_custom",
					),
				)
				.addStringOption((input) =>
					input
						.setName("embedtitle")
						.setNameLocalizations(
							client.lang.getNameLocalizations(
								"ticketmenu_custom_embedtitle",
							),
						)
						.setRequired(true)
				...
				.addStringOption((input) =>
					input
						...
						.setNameLocalizations(
							client.lang.getNameLocalizations(
								"ticketmenu_custom_embeddescription",
							),
						)
						.setRequired(true)
		...
		.addSubcommand((input) =>
			input
				.setName("selector")
				.setNameLocalizations(
					client.lang.getNameLocalizations("ticketmenu_selector"),
				)
	...
	async execute(interaction: ChatInputCommandInteraction): Promise<boolean> {
		...
		if (interaction.channel?.isSendable()) {
			...
			await interaction.followUp(
				client.lang.getResponse(interaction.locale, "ticketmenu_sent"),
			);
		} else {
			await interaction.followUp(
				client.lang.getResponse(
					interaction.locale,
					"ticketmenu_cant_send",
				),
			);
		}
