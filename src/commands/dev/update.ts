...
export default {
	...
	async execute(interaction: ChatInputCommandInteraction) {
		...
		const updateAttachment = interaction.options.getAttachment(
			"package",
			true,
		);
		if (updateAttachment.contentType == "application/zip") {
			const reply = await interaction.followUp({
				embeds: [updateEmbed.setTimestamp(new Date())],
			});
			const updateRes = await fetch(updateAttachment.url);
			const updateBuffer = await updateRes.arrayBuffer();
			await reply.edit({
				embeds: [areYouSureEmbed.setTimestamp(new Date())],
				components: [
					new ActionRowBuilder<ButtonBuilder>().addComponents(
						new ButtonBuilder()
							.setLabel("Zaktualizuj")
							.setCustomId("updateConfirm")
							.setStyle(ButtonStyle.Success),
					),
				],
			});
			const collector = reply.awaitMessageComponent({
				componentType: ComponentType.Button,
				time: 10_000,
				filter: (interaction2, _) =>
					interaction2.user.id == interaction.user.id &&
					interaction2.customId == "updateConfirm",
			});
			collector
				.then(async () => {
					...
					if (updated) {
						...
						updateEmbed.setDescription(
							`Zaktualizowano w ${updateEmbed.data.timestamp ? (date.getTime() - new Date(updateEmbed.data.timestamp).getTime()) / 1000 : "???"}s.\nKończenie procesu...`,
						);
						...
						updateEmbed.setDescription(
							`Coś poszło nie tak podczas aktualizacji.\nSprawdź terminal.`,
						);
						updateEmbed.setTimestamp(date);
				...
				.catch(async () => {
					await reply.edit({
						embeds: [updateCancelledEmbed.setTimestamp(new Date())],
						components: [],
					});
				});
		} else {
			await interaction.followUp({
				embeds: [wrongContentTypeEmbed.setTimestamp(new Date())],
			});
		}
		return true;
	},
