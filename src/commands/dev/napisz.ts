...
export default {
	...
	async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (channelId) channel = await interaction.client.channels.fetch(channelId)
        if (channel && channel.isSendable()) {
            await channel.send(interaction.options.getString("tekst", true));
            await interaction.followUp({ content: "`✅ Wysłano.`", flags: MessageFlags.Ephemeral });
        } else {
            await interaction.followUp({ content: "`❌ Kanał nie istnieje lub nie da się na nim pisać.`", flags: MessageFlags.Ephemeral });
        }
        return true;
	}
}