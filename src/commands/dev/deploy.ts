...
export default {
	data: new SlashCommandBuilder()
		.setName("deploy")
		.setDescription(
			"Deployuje komendy. (nwm jak to sie nazywa po polsku lol)",
		)
		.addSubcommand((subcommand) =>
			subcommand
				.setName("serwer")
				.setDescription(
					"Deployuj komendy na serwerze. (Najlepiej na swój prywatny)",
				)
				.addStringOption((option) =>
					option.setName("id").setDescription("ID serwera."),
				),
		)
		.addSubcommand((subcommand) =>
			subcommand
	...
	async execute(interaction: ChatInputCommandInteraction, dbUser: User) {
		...
		if (interaction.options.getSubcommand() == "serwer") {
			await deployGuild(
				interaction.options.getString("id") || config.guildId,
			);
