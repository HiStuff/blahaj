...
export default {
	...
	async execute(interaction: ChatInputCommandInteraction) {
		...
		if (interaction.options.getSubcommand(true) == "usun") {
			...
			try {
				await database.ticketTheme.delete({ where: { guildId: interaction.guildId, id: interaction.options.getString("idmotywu", true) } });
				res.edit("`✅ Usunięto!`");
			} catch(err: any) {
				console.log(err)
				if (err.code == "P2025") {
			...
			// LISTA
			// LISTA
			let themes = await database.ticketTheme.findMany({ where: { guildId: interaction.guildId } });
			let content = ""
			if (themes.length > 0) {
                ...
                if (data) {
                    ...
                    if (res.channel.isSendable()) {
                        const ticketTheme = await database.ticketTheme.create({
                            data: {
                                selectionOptionName: interaction.options.getString("nazwawyboru"),
                                selectionOptionIcon: interaction.options.getString("emotkawyboru"),
                                channelName: interaction.options.getString("nazwakanalu", true),
                                guildId: interaction.guildId,
                                ticketIntro: JSON.stringify(msg)
                            }
                        });
                        await res.edit(`\`✅ Stworzono motyw ticketa z ID ${ticketTheme.id}, nazwą kanału #${interaction.options.getString("nazwakanalu", true)} i wyborem ${interaction.options.getString("emotkawyboru") || "⚫"} - ${interaction.options.getString("nazwawyboru") || "BRAK"}.\``);
                    }
                } else {
                    await res.edit("`⚠️ Podał*ś nieprawidłowy adres URL.`");
                }
            } catch(err) {
                await res.edit("`⚠️ Wystąpił błąd podczas przetwarzania. Upewnij się że podał*ś prawidłowy adres URL.`");
            }
		}
		return true;
	},
