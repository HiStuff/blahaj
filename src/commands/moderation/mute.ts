import { ActionRowBuilder, ChatInputCommandInteraction, DiscordAPIError, EmbedBuilder, GuildMember, InteractionContextType, ModalBuilder, PermissionFlagsBits, SlashCommandBuilder, SlashCommandUserOption, TextInputBuilder, TextInputStyle } from "discord.js";
import { Error, createErrorEmbed } from "../../utils/error.js";
import { getPenaltyDurationTimeString, getPenaltyTimeLeftString } from "../../utils/number.js";
...
export default {
	...
	async execute(interaction: ChatInputCommandInteraction) {
        ...
        let time = null;
        if (Number(timeString)) { time = Number(timeString) * 60 * 1000 };
        if (!member) {
            await interaction.followUp({ embeds: [ createErrorEmbed(Error.USER_IS_NULL) ] });
            return;
        }
...
Mija ${getPenaltyTimeLeftString(time)}
            ...
            await response.followUp(`Wyciszono ${guildMember.user} za \`${reason}\` na \`${getPenaltyDurationTimeString(time)}\`.`);
        } else {
            await response.followUp({ embeds: [ createErrorEmbed(Error.USER_NOT_MUTABLE) ] });
        }
	}
};